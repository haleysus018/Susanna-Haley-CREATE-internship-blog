...
## How I built this
I built 3 input areas where the user puts in the node,description, and amount of edges. Then I created multiple routes,routes define the URL path and what the server should do when someone visits that path, for each time someone pushes a button. I had the generate flowchart route happen when someone pushed generate flwochart button. This route sends a prompt to OpenAI to create mermaid code without any markdown or anything that might stop it from rendering on the webpage. then it sends back that mermaid code as a json object back into a textarea in the html page. Then you can edit it if you wish and then push render diagram which ll render as an svg file the mermaid code in that textarea and put it in a part of the html page. If you push the handdrawn button it will go to the handdrawn route. this route prompts the OpenAi API to generate a list of nodes and their properties and edges and hteir properties. Then i insert it into a box in the html page where you can't edit it but you can see it. This is for you to decide the positioning of the nodes and edges. 
## Diagram generator
...
## how I built this
I have a textarea in javascript where I can take the user input, which should be mermaid code and if it isn't it will throw an error, and put it into a variable. I then pass that variable through a function that uses mermaid's API render function to render the mermaid cod. I then put it in as an svg file and insert it into a specific place in the html web page. 
